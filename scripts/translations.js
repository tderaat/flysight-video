...
var I18N = {
  en: {
    ...
    'video.errLoadHevc': 'Could not load this video file.\n\nIts video track is H.265/HEVC ({fourcc}), which this browser cannot decode. Firefox advertises HEVC support on Windows but fails to decode it in practice.\n\nTwo options:\n1. Open FlySight Insights in Chrome or Edge, which do decode HEVC.\n2. Re-encode to H.264:\nffmpeg -i "{name}" -c:v libx264 -crf 20 -c:a aac -movflags +faststart out.mp4',
  ...
  de: {
    ...
    'video.errLoadHevc': 'Diese Videodatei konnte nicht geladen werden.\n\nDie Videospur ist H.265/HEVC ({fourcc}), was dieser Browser nicht dekodieren kann. Firefox gibt unter Windows HEVC-Unterstützung an, kann sie in der Praxis aber nicht dekodieren.\n\nZwei Möglichkeiten:\n1. Öffnen Sie FlySight Insights in Chrome oder Edge, die HEVC dekodieren.\n2. Kodieren Sie die Datei nach H.264 um:\nffmpeg -i "{name}" -c:v libx264 -crf 20 -c:a aac -movflags +faststart out.mp4',
  ...
  nl: {
    ...
    'video.errLoadHevc': 'Kan dit videobestand niet laden.\n\nDe videotrack is H.265/HEVC ({fourcc}), die deze browser niet kan decoderen. Firefox meldt op Windows wel HEVC-ondersteuning, maar kan het in de praktijk niet decoderen.\n\nTwee opties:\n1. Open FlySight Insights in Chrome of Edge, die HEVC wel decoderen.\n2. Hercodeer naar H.264:\nffmpeg -i "{name}" -c:v libx264 -crf 20 -c:a aac -movflags +faststart out.mp4',
  ...
  it: {
    ...
    'video.errLoadHevc': 'Impossibile caricare questo file video.\n\nLa traccia video è H.265/HEVC ({fourcc}), che questo browser non può decodificare. Firefox dichiara il supporto HEVC su Windows ma in pratica non riesce a decodificarlo.\n\nDue opzioni:\n1. Apri FlySight Insights in Chrome o Edge, che decodificano HEVC.\n2. Ricodifica in H.264:\nffmpeg -i "{name}" -c:v libx264 -crf 20 -c:a aac -movflags +faststart out.mp4',
